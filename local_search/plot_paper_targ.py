...
def plot(file, barrier_range, plot_name,min_max_range):
    ...
    for barrier in barrier_range:
        ...
        results = np.array(pickle.load(open('{}/targ/{}_dict.txt'.format(file, barrier), "rb")))
    ...
    plt.title('Direct LS, {}: Found target barriers'.format(plot_name))
    plt.savefig('{}/{}_local_direct_target.png'.format(file, file), dpi=600)
    plt.clf()
    ...
    plt.ylabel('Activation barrier / kcal mol$^{-1}$')
    plt.legend()
    plt.title('Direct LS, {}: Found min and max barriers'.format(plot_name))
    plt.savefig('{}/{}_local_direct_minmax.png'.format(file, file), dpi=600)
    plt.clf()
