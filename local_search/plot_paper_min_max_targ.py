...
def plot(file, barrier_range, plot_name, min_max_range):
    ...
    for barrier in barrier_range:
        ...
        results = np.array(pickle.load(open('{}/min_max_targ/{}_dict.txt'.format(file, barrier), "rb")))
        
        dev_means = np.mean(results[:, 2].astype(float), axis=0)[:480]
        dev_stds = np.std(results[:, 2].astype(float), axis=0)[:480]
    ...
    plt.title('Multiple LS, {}: Found target barriers'.format(plot_name))
    plt.savefig('{}/{}_local_multiple_target.png'.format(file, file), dpi=600)
    plt.clf()
    ...
    plt.ylabel('Activation barrier / kcal mol$^{-1}$')
    plt.legend()
    plt.title('Multiple LS, {}: Found min and max barriers'.format(plot_name))
    plt.savefig('{}/{}_local_multiple_minmax.png'.format(file, file), dpi=600)
    plt.clf()
