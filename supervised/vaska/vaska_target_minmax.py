...
def barriers_groups(data_file, csv_file):
	# NB: R1==B R2==C R3==A R4==A
	dataframe = pd.read_csv(data_file)
	csv_file = open(csv_file, "w", newline="")
	csv_writer = csv.writer(csv_file, delimiter=",")
	core = Chem.MolFromSmarts("[Ir]([*:1])([*:2])([*:3])[*:4]")
	csv_writer.writerow(["R1", "R2", "R3", "R4", "barrier"])
	for i in range(len(dataframe)):
		smiles = dataframe["smiles"].iloc[i]
		mol = Chem.MolFromSmiles(smiles)
		res, unmatched = rdRGD.RGroupDecompose([core], [mol], asSmiles=True)
		mol_data = [res[0]["R1"], res[0]["R2"], res[0]["R3"], res[0]["R4"], dataframe["barrier"].iloc[i]]
...
def get_data(csv_file, target_value=0.0, rstate=1):
	...
	# Move the minimum and maximum barriers to the end of the dataset, to make sure they are not in the initial sample
...
#barriers_groups("vaska_features_properties_smiles_filenames.csv", "vaska_barriers_groups.csv")

min_results = {}
max_results = {}
target_results = {}
barriers = [0.0, 5.0, 10.0, 15.0, 20.0, 25.0]
...
for label, fraction in zip(fraction_labels, fractions):
	...
	for target in barriers:
		...
		for rstate in range(1,51):
			print(80 * "-")
			print("Vaska", label, target, "kcal/mol", "rstate =", rstate)
			Xy = get_data("vaska_barriers_groups.csv", target_value=target, rstate=rstate)
...
results_file.write("def vaska_results():\n")
