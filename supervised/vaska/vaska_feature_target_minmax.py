...
def get_features(csv_file):
	...
	data = data.drop(["smiles", "filename"], axis=1)
	# Scale the feature values since some are quite large
	columns = list(data.columns)
	columns.remove("barrier")
	column_transformer = ColumnTransformer([("standard_scaler", StandardScaler(), columns)], remainder="passthrough")
...
Xy = get_features("vaska_features_properties_smiles_filenames.csv")
...
results_file.write("def vaska_feature_results():\n")
