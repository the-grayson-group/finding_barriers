...
import sys
sys.path.insert(0, "..")
from feature_results import vaska_feature_results
...
min_results, max_results, target_results = vaska_feature_results()
...
for fraction_label, fraction_title in zip(fraction_labels, fraction_titles):
	...
	ax.set_title("ML H$_2$ (features): Found target barriers" + ((" (" + fraction_title + ")") if fraction_title is not None else ""))
	...
	fig.savefig("vaska_" + fraction_label + "_target.png", bbox_extra_artists=(lgd,), bbox_inches="tight", dpi=600)
	plt.gcf().clear()
	...
	for barrier in min_results[fraction_label].keys():
		...
		ax.plot([12, 490], [2.0, 2.0], "--", linewidth=1.0, color="grey")
		ax.plot([12, 490], [max_barrier, max_barrier], "--", linewidth=1.0, color="black")
		ax.plot([12, 490], [24.0, 24.0], "--", linewidth=1.0, color="grey")
	ax.set_title("ML H$_2$ (features): Found min and max barriers" + ((" (" + fraction_title + ")") if fraction_title is not None else ""))
	ax.set_xlabel("Number of samples")
	ax.set_xticks(ticks=[12 * iter + 12 for iter in iterations if iter % 4 == 1])
	ax.set_ylabel("Activation barrier / kcal mol$^{-1}$")
	lgd = ax.legend(bbox_to_anchor=(1.04, 1))
	fig.savefig("vaska_" + fraction_label + "_minmax.png", bbox_extra_artists=(lgd,), bbox_inches="tight", dpi=600)
	plt.gcf().clear()
