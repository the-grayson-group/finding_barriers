...
import sys
sys.path.insert(0, "..")
from feature_results import ma_finger_results
...
min_results, max_results, target_results = ma_finger_results()
...
for fraction_label, fraction_title in zip(fraction_labels, fraction_titles):
	...
	ax.set_title("ML MA (fingerprints): Found target barriers" + ((" (" + fraction_title + ")") if fraction_title is not None else ""))
	...
	ax.set_title("ML MA (fingerprints): Found min and max barriers" + ((" (" + fraction_title + ")") if fraction_title is not None else ""))
	ax.set_xlabel("Number of samples")
	ax.set_xticks(ticks=[12 * iter + 12 for iter in iterations if iter % 4 == 1])
	ax.set_ylabel("Activation barrier / kcal mol$^{-1}$")
