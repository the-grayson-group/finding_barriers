...
def get_data(csv_file, target_value=0.0, rstate=1):
	...
	R1_inc = 0
	R2_inc = R1_len
	R3_inc = R2_len + R1_len
	R4_inc = R3_len + R2_len + R1_len
	R1_indices = {}
	R2_indices = {}
	R3_indices = {}
	R4_indices = {}
	Xy = np.zeros((len(data), R1_len + R2_len + R3_len + R4_len + 1))
	# An attempt at packing all this into a for loop was made, but was horribly unreadable,
	# so I've left it written out without looping over R1,...,R4 for the sake of clarity
...
for label, fraction in zip(fraction_labels, fractions):
	...
	for target in barriers:
		...
		for rstate in range(1,51):
			...
			Xy = get_data("barriers_groups.csv", target_value=target, rstate=rstate)
...
results_file.write("def ma_results():\n")
